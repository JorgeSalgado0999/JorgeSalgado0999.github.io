...
Each Experience entry keeps its condensed one-line description always visible, with an `accent`-colored "Show more (+N)" / "Show less" text button (chevron rotates 180° on open, underlines on hover) that reveals 3-4 fuller bullet points sourced from the résumé, using the same dot-bullet style as the Skills section's soft-skills list for consistency. The `(+N)` count and a differentiated `aria-label` (role + company, not just the shared visible label) exist because a `/impeccable critique` pass found six identical-sounding toggles with no signal of what's behind them and no way for a screen reader to tell them apart — fixed rather than left as a known gap. The button carries 12px vertical padding specifically to clear the 44px touch-target minimum without growing the visible text (that critique also caught this exact toggle regressing a class of bug already fixed elsewhere on the site). Height animates via Framer Motion (`height: 0 → auto`, gated by `useReducedMotion()`), each entry expands independently. Every bullet must trace back to `PRODUCT.md`'s résumé-backed evidence — same guardrail as the Credibility Strip. An entry may also carry a short italic `note` (e.g. Bradescard's) directly under its description when a date or fact needs a one-line clarification a skeptical reader would otherwise have to guess at — always visible, never hidden behind the toggle.
